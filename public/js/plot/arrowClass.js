...
var StraightArrow = function(viewer) {
	...
	this.pointImageUrl = "image/plot/point.png";
...
StraightArrow.prototype = {
	...
	startDraw: function() {
		...
		this.handler.setInputAction(function(evt) { //单机开始绘制
			...
			if ($this.positions.length == 3) {
                ...
                $this.arrowEntity.type = "StraightArrow";
				$this.state = -1;

                Bus.$emit("DrawSituationStraightArrowArrow_finish");
                Bus.$emit("addEntityPlot", {
                    plotType: 'Plot_SituationArrow',
                    entityId: $this.arrowEntity.id
                });
			}
    ...
    createByData2: function(positions, entityId, outlineShow, outlineColor, fillShow, fillColor) { //通过传入的经纬度数组 构建箭头
        this.state = -1;
        this.positions = [];
        var arr = [];
        for (var i = 0; i < positions.length; i++) {
            var cart3 = Cesium.Cartesian3.fromDegrees(positions[i][0], positions[i][1]);
            arr.push(cart3);
        }
        this.positions = arr;
        this.firstPoint = this.creatPoint(this.positions[1]);
        ...
        this.floatPoint = this.creatPoint(this.positions[2]);
        this.floatPoint.type = "floatPoint";
        this.arrowEntity = this.showArrowOnMap2(this.positions, entityId, outlineShow, outlineColor, fillShow, fillColor);
        this.firstPoint.show = false;
        this.floatPoint.show = false;
        this.arrowEntity.objId = this.objId;
        this.arrowEntity.type = "StraightArrow";
    ...
    showArrowOnMap2: function(positions, entityId, outlineShow, outlineColor, fillShow, fillColor) {
        var $this = this;
        var update = function() {
            if (positions.length < 2) {
                return null;
        ...
        return this.viewer.entities.add({
            ...
            polygon: new Cesium.PolygonGraphics({
                ...
                height:0,
                outline: outlineShow,
                outlineColor: CesiumUtil.rgbaToColor(outlineColor),
                fill: fillShow,
                material : CesiumUtil.rgbaToColor(fillColor)
            })
        });
    },
...
AttackArrow.prototype = {
	...
	startDraw: function() {
		...
		this.handler.setInputAction(function(evt) { //移动时绘制面
			...
			if ($this.positions.length >= 2) {
				if (!Cesium.defined($this.arrowEntity)) {
					...
					$this.arrowEntity = $this.showArrowOnMap($this.positions);
					$this.arrowEntity.objId = $this.objId;
                    $this.arrowEntity.type = "AttackArrow";
				} else {
					$this.positions.pop();
					$this.positions.push(cartesian);
				}
		...
		this.handler.setInputAction(function(evt) { //右击结束绘制
            ...
            Bus.$emit("DrawSituationAttackArrow_finish");
            Bus.$emit("addEntityPlot", {
                plotType: 'Plot_SituationArrow',
                entityId: $this.arrowEntity.id
            });
    ...
    createByData2: function(data, entityId, outlineShow, outlineColor, fillShow, fillColor) { //根据传入的数据构建箭头
        this.positions = []; //控制点
        this.state = -1; //state用于区分当前的状态 0 为删除 1为添加 2为编辑
        this.floatPoint = null;
        this.pointArr = []; //中间各点
	...
	startDraw: function() {
		...
		this.handler.setInputAction(function(evt) { //单机开始绘制
			...
			if ($this.positions.length > 4) { //结束绘制
                ...
                Bus.$emit("DrawSituationPincerArrow_finish");
                Bus.$emit("addEntityPlot", {
                    plotType: 'Plot_SituationArrow',
                    entityId: $this.arrowEntity.id
                });
		...
		this.handler.setInputAction(function(evt) { //移动时绘制面
			...
			if ($this.positions.length >= 2) {
				if (!Cesium.defined($this.arrowEntity)) {
					...
					$this.arrowEntity = $this.showArrowOnMap($this.positions);
					$this.arrowEntity.objId = $this.objId;
                    $this.arrowEntity.type = "PincerArrow";
				} else {
					$this.positions.pop();
					$this.positions.push(cartesian);
				}
