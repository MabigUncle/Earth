...
<script>
	import DrawPolygon from '../js/DrawPolygon.js'
	import CreatePolygon from '../js/CreatePolygon.js'
	export default {
		name: 'PolygonPlotTab',
		data() {
			return {
			...
			startPlot() {
				switch (this.plotType) {
					case 'plotByMap':
						...
						break;
					case 'plotByLonlat':
						let createPolygon = new CreatePolygon(this.lonlatFormArr);
						PlotBus.$emit("addEntityPlot", {
							plotType: 'Plot_Polygon',
							entityId: createPolygon.finalEntityId
						});
						this.lonlatFormArr = [];
						break;
					case 'plotByDms':
						let lonlatFormArr = CesiumUtil.dmsArrToDegreeArr(this.dmsFormArr);
						let createPolygon2 = new CreatePolygon(lonlatFormArr);
						PlotBus.$emit("addEntityPlot", {
							plotType: 'Plot_Polygon',
							entityId: createPolygon2.finalEntityId
						});
						this.dmsFormArr = [];
						break;
				}
